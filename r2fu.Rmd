...
saveRDS(r2fu.raw.data,file="r2fu.long.data.RDS")
...
saveRDS(r2fu.raw.data,file="r2fu.wide.data.RDS")
...
## (aaas_ad ~ group * time + (1 | subject),data=r2fu.long.data[nlg3,])
...
performance::check_model(aaas_ad.lmerB)
...
- Wait list is not ideal as control (although apparently regressing to the mean in for all primary outcomes)
- De som väntelista med som uppföljning saknas inte slumpmässigt, kolla ev om de som inte svarat skiljt sig på något sätt (anova: vad är relevant att testa, testa endast de som är relevanta)
