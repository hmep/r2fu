...
digraph G {
  ...
  leads -> { screening_completers screening_non_completers }
  ...
  subgraph cluster_selfhelp { margin=8; labelloc="b"; label="Selfhelp\nn = 70 - 3" ; selfhelp_dur -> selfhelp_post -> selfhelp_fu; } 
  subgraph cluster_support { margin=8; labelloc="b"; label="Support\nn = 70 - 1" ; support_dur -> support_post -> support_fu; } 
  subgraph cluster_waitlist { margin=8; labelloc="b"; label="Waitlist\nn = 70 - 2" ; waitlist_dur -> waitlist_post; } 
  
  #subgraph cluster_selfhelp { margin=12; labelloc="t"; label="Selfhelp\nn = 70 - 3" ; selfhelp_pre -> selfhelp_dur -> selfhelp_post -> selfhelp_fu; } 
  #subgraph cluster_support { margin=12; labelloc="t"; label="Support\nn = 70 - 1" ; support_pre -> support_dur -> support_post -> support_fu; } 
  #subgraph cluster_waitlist { margin=12; labelloc="t"; label="Waitlist\nn = 70 - 2" ; waitlist_pre -> waitlist_dur -> waitlist_post; } 

  { selfhelp_fu support_fu waitlist_post } -> itt

  leads[label=<Leads from social media<BR/><I>n</I> = 657>]
  screening_completers[label="Completed screening\nn = 464"]
  excluded[label="Excluded from study\nn = 126, of which\nPHQ-9 cutoff (≥ 15): n = 77\nConcurrent therapy: n = 29\nRecent adjustment of psychotropic drugs: n = 7\nLow motivation/lack of time: n = 13"]
  screening_non_completers[label="Did not complete screening\nn = 193"]
  invited[label="Invited to participate in study\nn = 338"]
  invitation_non_responders[label="Declined or did not respond\nn = 85"]
  invitation_responders[label="Accepted invitation\nn = 253"]
  trimmed[label="Excluded by randomization\nn = 43"]
  keepers[label="Included by randomization\nn = 210"]
  randomization[label="Randomization to condition\nn = 70 each"]
  #adjusted_inclusion[label="Included\nn = 205"]
  #adjusted_exclusion[label="Excluded after randomization\nn = 5"]
  #selfhelp_pre[label=""]
  selfhelp_dur[label="Completed 4 week measurement\nn = 44"]
  selfhelp_post[label="Completed post measurement\nn = 35"]
  selfhelp_fu[label="Completed 1 year follow-up\nn = x"]
  #support_pre[label=""]
  support_dur[label="Completed 4 week measurement\nn = 48"]
  support_post[label="Completed post measurement\nn = 50"]
  support_fu[label="Completed 1 year follow-up\nn = x"]
  #waitlist_pre[label=""]
  waitlist_dur[label="Completed 4 week measurement\nn = 58"]
  waitlist_post[label="Completed post measurement\nn = 61"]
  itt[label="Intention to treat\nn = 205"]
}
EOF
...
```{r flowchart, out.height="70%", fig.align="center", fig.cap='(ref:flow)'}

knitr::include_graphics("flowchart-crop.pdf")
...
To improve reproducibility, only open-source and free tools were used in the analysis of data. Anonymized data and code are tracked at https://github.com/hmep/respektikvadrat (original master thesis study) and https://github.com/hmep/r2fu respectively (present study, which builds on the original study and includes follow-up measurements). The final versions are deposited at  https://doi.org/10.17045/sthlmuni.12148785 [@R2data2020] and (new figshare repository) respectively.
