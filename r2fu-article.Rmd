...
```{r emm.secondary.plots, fig.width = 10, fig.asp = 0.4, echo = F, fig.cap = 'Plots of estimated fixed effects for secondary syndromal outcome measures. Participation in the Support and Self-help conditions lead to signficant symptom alleviation between the Pre and Post as well as between Pre and 1-year Follow-up time points for depression and social anxiety, as captured with their respective measures. As with the transdiagnostic measures of assertive behaviors, the difference between the two time points was small or statistically inappreciable for both groups and both measures. The Waitlist control group did not significantly change between time points for either measure. However, to counteract even the slightest nocebo effect of Waitlist in the between-group comparisons the most conservative estimate for the Waitlist control condition was used; see dotted line.'}
...
Checking for any signs of reliable harm, a significant difference between groups in clinical significant change was found for the AAA-S Aggressive measure, `r report.kw(aaas_ag.pre.post.kw.harmed)`, as $n$ = $3$ participants ($4$\%) had reached below the threshold for the category 'harmed' while waiting to enter into treatment (see Table 2). However, post hoc testing did not reveal any significant pairwise differences between any of the treatment groups and the Waitlist, `r xprnot(aaas_ag.pre.post.kw.harmed$altP.adjusted[2])` and `r xprnot(aaas_ag.pre.post.kw.harmed$altP.adjusted[1])`. No other instances of suspected reliable harm were identified. 

(Utöka också med reliable deterioration?)
