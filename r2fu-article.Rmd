...
within.effsize <- function(lmerobj,effrow,direction,emmpairsrow=NULL) {
    ...
    linebreak(paste0(
      ...
      " [",
      xresult(rout(lmerobj)$eff.by.group$upper.CL[effrow] * direction, 2),
      ", ",
      xresult(rout(lmerobj)$eff.by.group$lower.CL[effrow] * direction, 2),
      "]",
      stars
    ), align = "c")
}

between.effsize <- function(lmerobj, row, direction, emmpairsrow=NULL) {
    stars <-  if(is.null(emmpairsrow)) {} else {
      xpss(rout(lmerobj)$emmpairs$p.value[emmpairsrow])
    }
  linebreak(paste0(
    xresult(rout(lmerobj)$eff.by.time$effect.size[row] * direction, 2),
    " [",
    xresult(rout(lmerobj)$eff.by.time$upper.CL[row] * direction, 2),
    ", ",
    xresult(rout(lmerobj)$eff.by.time$lower.CL[row] * direction, 2),
    "]",
    stars
  ),
  align = "c")
...
phq_sum.effsizes <-
  c(
    ...
    between.effsize(phq_sum.lmer,48,-1,7), # Pre Waitlist - (Follow-up Self-help); levels(r2fu.long.data$time)[which.min(phq_sum.emm.plot$data$predicted[c(1,4,7)])]
    ...
    between.effsize(phq_sum.lmer,48,-1,11), # Pre Waitlist - (Follow-up Support); levels(r2fu.long.data$time)[which.min(phq_sum.emm.plot$data$predicted[c(1,4,7)])]
...
gad_sum.effsizes <-
  c(
    ...
    between.effsize(gad_sum.lmer,48,-1,NULL), # During Waitlist - (Follow-up Self-help) ; levels(r2fu.long.data$time)[which.min(gad_sum.emm.plot$data$predicted[c(1,4,7)])]
    ...
    between.effsize(gad_sum.lmer,48,-1,NULL), # During Waitlist - (Follow-up Support); levels(r2fu.long.data$time)[which.min(gad_sum.emm.plot$data$predicted[c(1,4,7)])]
...
lsas_sum.effsizes <-
  c(
    ...
    between.effsize(lsas_sum.lmer,48,-1,17), # During Waitlist - (Follow-up Self-help) ; levels(r2fu.long.data$time)[which.min(lsas_sum.emm.plot$data$predicted[c(1,4,7)])]
    ...
    between.effsize(lsas_sum.lmer,48,-1,21), # During Waitlist - (Follow-up Support); levels(r2fu.long.data$time)[which.min(lsas_sum.emm.plot$data$predicted[c(1,4,7)])]
