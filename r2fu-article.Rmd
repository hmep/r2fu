...
effsize.tabrow2 <- function(outobj, contrast, direction, stars = "stars") {
  ...
  stopifnot(all(outobj$emmpairs[-c(3, 13, 22, 31:38), ][, 1] == outobj$eff.by.group[, 1]))
...
Both treatment groups showed sustained within-group effects on assertiveness at 1 year Follow-up when compared to the Pre-treatment measurement time point, measured with AAA-S Adaptive and the RAS, suggesting that assertive behaviors were still manifest well beyond the end of participation in the intervention. However, only in the Guided self-help group was the Post to Follow-up comparison in itself significant for AAA-S Adaptive and the RAS, `r report.text2(aaas_ad.out, "Post Support - (Follow-up Support)", -1)` and `r report.text2(rathus.out, "Post Support - (Follow-up Support)", -1)` respectively. For the other comparisons, there was no difference between Post and Follow-up time points in either group, from `r xprnot(rathus.out$emmpairs$p.value[52])` to `r xprnot(aaas_ag.out$emmpairs$p.value[66])`. This indicates, with regards to adaptive assertiveness, that therapist support provided some benefit to participants' ability to generalize behaviors beyond the duration of the intervention. Table 1 summarizes significance tests of estimated marginal mean differences and effect sizes. 

Among the secondary measures of syndromal symptoms, depression as captured with PHQ-9 had decreased significantly from Pre only in the Guided self-help group, and only at Follow-up, `r report.text2(phq_sum.out, "Pre Support - (Follow-up Support)", 1)`, possibly indicating that therapist support benefits depressed participants on a longer rather than shorter time scale. Symptoms of social anxiety, however, decreased significantly for all four combinations of time points and groups. In the Self-help group, the effect from Pre to Post was `r report.text2(lsas_sum.out, "(Pre Self-help) - (Post Self-help)", 1)`, increasing to `r report.text2(lsas_sum.out, "(Pre Self-help) - (Follow-up Self-help)", 1)` Pre to Follow-up. In the Guided self-help group, the effect from Pre to Post was `r report.text2(lsas_sum.out, "Pre Support - Post Support", 1)`, increasing to `r report.text2(lsas_sum.out, "Pre Support - (Follow-up Support)", 1)` Pre to Follow-up. Although effect sizes for LSAS-SR was larger Pre--Follow-up than Pre--Post, there was no significant difference between Post and Follow-up in either group, `r xprnot(lsas_sum.out$emmpairs$p.value[52])` for Self-help and `r xprnot(lsas_sum.out$emmpairs$p.value[66])` for Guided self-help, suggesting that change in relief from symptoms of social phobia plateaued earlier than did change in manifest expressions of adaptive assertiveness. The within-group effects taken together indicate that the intervention was effective at addressing more contextually specific anxiety-induced avoidance and escape behaviors, replacing them with already desired assertive behaviors, to a larger extent than it was effective at helping participants struggling with generalized anxiety or depression to formulate reasons to start responding more flexibly.
