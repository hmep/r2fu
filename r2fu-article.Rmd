...
For the primary transdiagnostic measures of skillful behaviors it is notable that both treatment groups Self-help and Guided self-help improved significantly during the course of the intervention, showing large within-group effects with regards to AAA-S Adaptive from Pre to Post, `r report.text(aaas_ad.lmer, 29, -1, 40)`, and `r report.text(aaas_ad.lmer, 51, -1, 62)` respectively. No effect was found for the Waitlist condition, `r xprnot(rout(aaas_ad.lmer)$emmpairs$p.value[2])`. For the RAS, significant large effects on assertiveness were also found for the Self-help group, `r report.text(rathus.lmer, 29, -1, 40)`, and the Guided self-help group, `r report.text(rathus.lmer, 51, -1, 62)`. Participants in the Waitlist condition did not change between Pre and Post with regards to the RAS, nor did any other remaining measure, all `r xprnot(rout(rathus.lmer)$emmpairs$p.value[2])`. Both treatment groups showed significant and further inflated within-group effects for on assertiveness, measured with AAA-S Adaptive and the RAS, at 1 year Follow-up when compared to the Pre-treatment measurement time point, suggesting that assertive behaviors continued to generalize well after the end of participation in the treatment groups (see Table 1 for effect sizes). 

Among the secondary measures of syndromal symptoms ... (lyft viss effekt för depression mätt med PHQ-9 at Follow-up, lyft stor effekt på symtom för social ångest)

Adding to the picture that adaptive assertivness behaviors increased and depressive and social phobic symptoms decreased as a consequence of participation in the intervention, a significant number of participants moved across the cutoff for reliable and clinically significant for several measures ... (chi2-test, post-hoc-test)
...
Compared to the Waitlist condition, both the Self-help and the Guided self-help conditions showed significant and large effects at the Post timepoint, ... (utveckla mera)
