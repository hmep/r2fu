...
rout <- function(lmerobj) {
  ...
  x.eff.by.group <-
    eff_size(x.emm.by.group[effsel.group], sigma = x.sd, edf = df.residual(lmerobj))
  x.eff.by.time <-
    eff_size(x.emm.by.time[effsel.time], sigma = x.sd, edf = df.residual(lmerobj))
  return(
    list(
...
within.effsize <-
  function(lmerobj, effrow, direction, emmpairsrow = NULL) {
    stars <-  if (is.null(emmpairsrow)) {
    } else {
...
effsize.tabrow <- function(lmerobj, contrast, direction) {
  ...
  stars <-
    xpss(rout(lmerobj)$emmpairs[-c(3, 13, 22, 31:38), ]$p.value[effrow])
  #}
  if (direction == -1) {
    lower = rout(lmerobj)$eff.by.group$upper.CL[effrow] * direction
    upper = rout(lmerobj)$eff.by.group$lower.CL[effrow] * direction
...
#aaas_ad.out$emm.by.group[aaas_ad.out$emm.by.group$group == "Support" & aaas_ad.out$emm.by.group$time == "Pre",]

report.text2 <-
  function(outobj, contrast, direction = 1) {
    excluderows <- -c(3, 13, 22, 31:38)
    effrow <- which(outobj$eff.by.group$contrast == contrast)
    if (direction == -1) {
      lower = outobj$eff.by.group$upper.CL[effrow] * direction
      upper = outobj$eff.by.group$lower.CL[effrow] * direction
    } else {
      lower = outobj$eff.by.group$lower.CL[effrow] * direction
      upper = outobj$eff.by.group$upper.CL[effrow] * direction
    }
    x <- paste0(
      "$\\Delta M$ = ",
      xresult(outobj$emmpairs[excluderows,]$estimate[effrow] * direction, 1),
      #", $t$\\(_{",
      ", $t$($",
      xresulttext(outobj$emmpairs[excluderows,]$df[effrow], 0),
      #"}\\) = ",
      "$) = ",
      xresult(outobj$emmpairs[excluderows,]$t.ratio[effrow], 2),
      ", ",
      xpr(outobj$emmpairs[excluderows,]$p.value[effrow]),
      ", $ES$ = ",
      xresult(outobj$eff.by.group$effect.size[effrow] * direction, 2),
      ", 95\\% CI [",
      xresult(lower, 2),
      ", ",
      xresult(upper, 2),
...
effsize.tabrow <- function(lmerobj, contrast, direction) {
  effrow <-
    which(rout(aaas_ad.lmer)$eff.by.group$contrast == contrast)
  stopifnot(all(rout(lmerobj)$emmpairs[-c(3, 13, 22, 31:38), ][, 1] == rout(lmerobj)$eff.by.group[, 1]))
  stars <-
    xpss(rout(lmerobj)$emmpairs[-c(3, 13, 22, 31:38), ]$p.value[effrow])
  if (direction == -1) {
...
Among the secondary measures of syndromal symptoms, depression as captured with PHQ-9 had decreased significantly from Pre only in the Guided self-help group, and only at Follow-up, `r report.text2(phq_sum.out, "Pre Support - (Follow-up Support)", 1)`, possibly indicating that therapist support benefits depressed participants on a longer rather than shorter time scale. Symptoms of social anxiety, however, decreased significantly for all four combinations of time points and groups. In the Self-help group, the effect from Pre to Post was `r report.text2(lsas_sum.out, "(Pre Self-help) - (Post Self-help)", 1)`, increasing to `r report.text2(lsas_sum.out, "(Pre Self-help) - (Follow-up Self-help)", 1)` Pre to Follow-up. In the Guided self-help group, the effect from Pre to Post was `r report.text2(lsas_sum.out, "Pre Support - Post Support", 1)`, increasing to `r report.text2(lsas_sum.out, "Pre Support - (Follow-up Support)", 1)` Pre to Follow-up. Although effect sizes for LSAS-SR was larger Pre--Follow-up than Pre--Post, there was no significant difference between Post and Follow-up in either group, `r xprnot(rout(lsas_sum.lmer)$emmpairs$p.value[52])` for Self-help and `r xprnot(rout(lsas_sum.lmer)$emmpairs$p.value[66])` for Guided self-help, suggesting that change in relief from symptoms of social phobia plateaued earlier than did change in manifest expressions of adaptive assertiveness. The within-group effects taken together indicate that the intervention was effective at addressing more contextually specific anxiety-induced avoidance and escape behaviors, replacing them with already desired assertive behaviors, to a larger extent than it was effective at helping participants struggling with generalized anxiety or depression to formulate reasons to start responding more flexibly.
...
Again comparing against Waitlist, there was no significant effect of the Self-help condition on reduction of social anxiety symptoms measured with LSAS-SR at Post-treatment, `r xprnot(rout(lsas_sum.lmer)$emmpairs$p.value[25])`, however, a large effect was found at Follow-up, `r report.text2(lsas_sum.out, "During Waitlist - (Follow-up Self-help)", 1)`, comparing against the lowest value collected (at During, half-way through the intervention, in order to cancel out any possible nocebo effect of the Waitlist condition; see Figure 3 for a graphic identification of time point). No effect on alleviation of depressive symptoms measured with PHQ-9 at Post was found comparing Waitlist and Self-help groups, `r  xprnot(rout(phq_sum.lmer)$emmpairs$p.value[25])`. Some evidence was found suggesting that depressive participants might benefit from therapist support, in that the Waitlist versus Guided self-help comparison for PHQ-9 revealed a medium to large sized effect at Post, `r report.text2(phq_sum.out, "Post Waitlist - Post Support", 1)`. However, this effect dissipated when the Follow-up comparison was instead made against the most conservative value collected in the Waitlist group (in this case Pre instead of Post; see Figure 3 for a graphic exploration of a possible nocebo effect, that is symptomatic worsening over time due to self-fulfilling expectations of hopelessness among participants waiting in line for treatment). Thus, the between-group effect on depression at Post should be interpreted with caution.
