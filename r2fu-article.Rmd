...
aaas_ad.anova <- anova(aaas_ad.lmer,type=3) # Significant interaction, calls for post-hoc analysis
aaas_ad.emm.plot <-
  plot(
...
aaas_ag.anova <- anova(aaas_ag.lmer,type=3) # No significant interaction
aaas_ag.emm.plot <-
  plot(
...
rathus.anova <- anova(rathus.lmer,type=3) # Significant interaction, calls for post-hoc analysis
rathus.emm.plot <-
  plot(
...
lsas_sum.anova <- anova(lsas_sum.lmer,type=3) # Significant interaction, calls for post-hoc analysis
lsas_sum.emm.plot <-
  plot(
...
phq_sum.anova <- anova(phq_sum.lmer,type=3) # Significant interaction, calls for post-hoc analysis
phq_sum.emm.plot <-
  plot(
...
gad_sum.anova <- anova(gad_sum.lmer,type=3) # No significant interaction
gad_sum.emm.plot <-
  plot(
...
well.anova <- anova(well.lmer,type=3) # Significant interaction, calls for post-hoc analysis
well.emm.plot <-
  plot(
...
knitr::kable(clinsig.table, caption="Clinical significance summary of the number (and proportion in \\%) of participants that changed reliably and moved from the clinical to the functional population from Pre-treatment to Post- and 1-year Followup-time-points respectively (rows named ”Recovered”). For missing values (i.e. caused by drop-outs), the last collected value was moved forward to the next measurement time point, in order to respect the intention to treat principle.", booktabs = T, format = "latex", align="rccccc", escape = F) %>%
  ...
  # pack_rows(paste0(measurementnames[1],xpsshack(0.02)), 1, 5, bold = F, escape = F) %>%
  # pack_rows(paste0(measurementnames[2],xpsshack(0.91)), 6, 10, bold = F, escape = F) %>%
  # pack_rows(paste0(measurementnames[3],xpsshack(0)), 11, 15, bold = F, escape = F) %>%
  # pack_rows(paste0(measurementnames[4],xpsshack(0)), 16, 20, bold = F, escape = F) %>%
  # pack_rows(paste0(measurementnames[5],xpsshack(0.08)), 21, 25, bold = F, escape = F) %>%
  # pack_rows(paste0(measurementnames[6],xpsshack(0)), 26, 30, bold = F, escape = F) %>%
