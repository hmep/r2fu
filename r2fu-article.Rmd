...
xresult <-
  function(x, d = 2) {
    paste("$",
          apa(value = x, decimals = d, leading = F),
          "$",
          sep = "")
  }
...
xpr <- function(x, d = 3) {
  paste("$p$",
        ifelse(x < 0.001, "< $.001$", ifelse(
          x < 0.05,
          paste0("= $", apa(value = x, decimals = d, leading = F), "$"),
          ifelse(x > 0.05, "DO NOT REPORT", "= $.05$")
        )))
}

xprnot.off <- function(x) {
  paste("$p$",
        ifelse(x < 0.001, "< $0.001$", ifelse(
          x < 0.05,
          paste0(" = $", prettyNum(
...
Data was collected using the below measures at four time points: Pre treatment (Week 0), midway During the intervention (Week 4), Post treatment (Week 8), and at 1 year Follow-up.
...
aaas_ad.anova <- anova(aaas_ad.lmer, type=3, ddf="Kenward-Roger") # Significant interaction, calls for pairwise comparisons
aaas_ad.emm.plot <-
  plot(
...
rathus.anova <- anova(rathus.lmer, type=3, ddf="Kenward-Roger") # Significant interaction, calls for pairwise comparisons
rathus.emm.plot <-
  plot(
...
lsas_sum.anova <- anova(lsas_sum.lmer, type=3, ddf="Kenward-Roger") # Significant interaction, calls for pairwise comparisons
lsas_sum.emm.plot <-
  plot(
...
phq_sum.anova <- anova(phq_sum.lmer, type=3, ddf="Kenward-Roger") # Significant interaction, calls for pairwise comparisons
phq_sum.emm.plot <-
  plot(
...
well.anova <- anova(well.lmer, type=3, ddf="Kenward-Roger") # Significant interaction, calls for pairwise comparisons
well.emm.plot <-
  plot(
...
```{r emm.primary.plots, fig.width = 10, fig.asp = 0.4, echo = F, fig.cap = 'Plots of estimated fixed effects for two of the primary transdiagnostic scales used to measure skillful, assertive behaviors. Participants’ estimated means for both measures exhibit increasing levels of assertiveness during the course of treatment in the Support and Self-help groups, with negligable differences between the two treatment conditions. The trends continue beyond the Post time point, showing further increased estimated mean levels of assertive behaviors at 1-year Follow-up. The Waitlist group’s means recovered slightly, although not significantly, between the Pre and Post time points, for both measures.'}

(aaas_ad.emm.plot + theme(plot.margin = unit(c(0,8,0,0), "pt"))) +
  #(aaas_ag.emm.plot + theme(plot.margin = unit(c(0,0,0,8), "pt"))) +
  (rathus.emm.plot + theme(plot.margin = unit(c(0,0,0,8), "pt"))) #+ 
  #guide_area() + plot_layout(guides = 'collect') + plot_layout(ncol = 2) 

```

```{r emm.secondary.plots, fig.width = 10, fig.asp = 0.4, echo = F, fig.cap = 'Plots of estimated fixed effects for secondary syndromal outcome measures. Participation in the Support and Self-help conditions lead to significant symptom alleviation between the Pre and Post as well as between Pre and 1-year Follow-up time points for depression and social anxiety, as captured with their respective measures. As with the transdiagnostic measures of assertive behaviors, the difference between the two time points was small or statistically inappreciable for both groups and both measures. The Waitlist control group did not significantly change between time points for either measure. However, to counteract even the slightest nocebo effect of Waitlist in the between-group comparisons the most conservative estimate for the Waitlist control condition was used; see dotted line.'}

(phq_sum.emm.plot + theme(plot.margin = unit(c(0,8,0,0), "pt"))) +
  #(gad_sum.emm.plot + theme(plot.margin = unit(c(0,0,0,8), "pt"))) +
  (lsas_sum.emm.plot + theme(plot.margin = unit(c(0,0,0,8), "pt"))) #+ 
  #guide_area() + plot_layout(guides = 'collect') + plot_layout(ncol = 2) 
...
aaas_ad.pre.post.kw <- dunn.test(list(
  clinsigraw("aaas_ad", "higher", "Pre", "Post", xG3)$recovered,
  clinsigraw("aaas_ad", "higher", "Pre", "Post", xG1)$recovered,
  clinsigraw("aaas_ad", "higher", "Pre", "Post", xG2)$recovered
...
#   clinsigraw("aaas_ad", "higher", "Pre", "Post", xG3)$worsened,
#   clinsigraw("aaas_ad", "higher", "Pre", "Post", xG1)$worsened,
#   clinsigraw("aaas_ad", "higher", "Pre", "Post", xG2)$worsened
# ), method="bonferroni", altp=TRUE)
# No worsening in any group, nothing to test!

aaas_ag.pre.post.kw <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG3)$recovered,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG1)$recovered,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG2)$recovered
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 0.1847, df = 2, p-value = 0.91

aaas_ag.pre.post.kw.harmed <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG3)$harmed,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG1)$harmed,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG2)$harmed
), method="bonferroni", altp=TRUE)
...
aaas_ag.pre.post.kw.worsened <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG3)$worsened,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG1)$worsened,
  clinsigraw("aaas_ag", "higher", "Pre", "Post", xG2)$worsened
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 3.1136, df = 2, p-value = 0.21

rathus.pre.post.kw <- dunn.test(list(
  clinsigraw("rathus", "higher", "Pre", "Post", xG3)$recovered,
  clinsigraw("rathus", "higher", "Pre", "Post", xG1)$recovered,
  clinsigraw("rathus", "higher", "Pre", "Post", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
#   clinsigraw("rathus", "higher", "Pre", "Post", xG3)$worsened,
#   clinsigraw("rathus", "higher", "Pre", "Post", xG1)$worsened,
#   clinsigraw("rathus", "higher", "Pre", "Post", xG2)$worsened
# ), method="bonferroni", altp=TRUE)
# No worsening in any group, nothing to test!

phq_sum.pre.post.kw <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG3)$recovered,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG1)$recovered,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
phq_sum.pre.post.kw.harmed <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG3)$harmed,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG1)$harmed,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 0.9316, df = 2, p-value = 0.63

phq_sum.pre.post.kw.worsened <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG3)$worsened,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG1)$worsened,
  clinsigraw("phq_sum", "lower", "Pre", "Post", xG2)$worsened
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 5.0582, df = 2, p-value = 0.08

gad_sum.pre.post.kw <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG3)$recovered,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG1)$recovered,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG2)$recovered
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.0239, df = 2, p-value = 0.36

gad_sum.pre.post.kw.harmed <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG3)$harmed,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG1)$harmed,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.9776, df = 2, p-value = 0.23

gad_sum.pre.post.kw.worsened <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG3)$worsened,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG1)$worsened,
  clinsigraw("gad_sum", "lower", "Pre", "Post", xG2)$worsened
), method="bonferroni", altp=TRUE)
...
lsas_sum.pre.post.kw <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG3)$recovered,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG1)$recovered,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
lsas_sum.pre.post.kw.harmed <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG3)$harmed,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG1)$harmed,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.0597, df = 2, p-value = 0.36

lsas_sum.pre.post.kw.worsened <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG3)$worsened,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG1)$worsened,
  clinsigraw("lsas_sum", "lower", "Pre", "Post", xG2)$worsened
), method="bonferroni", altp=TRUE)
...
aaas_ad.pre.fu.kw <- dunn.test(list(
  clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
#   clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG3)$worsened,
#   clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG1)$worsened,
#   clinsigraw("aaas_ad", "higher", "Pre", "Follow-up", xG2)$worsened
# ), method="bonferroni", altp=TRUE)
# No worsening in any group, nothing to test!

aaas_ag.pre.fu.kw <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 1.7287, df = 2, p-value = 0.42

aaas_ag.pre.fu.kw.harmed <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG3)$harmed,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG1)$harmed,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 3.5677, df = 2, p-value = 0.17

aaas_ag.pre.fu.kw.worsened <- dunn.test(list(
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG3)$worsened,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG1)$worsened,
  clinsigraw("aaas_ag", "higher", "Pre", "Follow-up", xG2)$worsened
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 4.0919, df = 2, p-value = 0.13

rathus.pre.fu.kw <- dunn.test(list(
  clinsigraw("rathus", "higher", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("rathus", "higher", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("rathus", "higher", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
#   clinsigraw("rathus", "higher", "Pre", "Follow-up", xG3)$worsened,
#   clinsigraw("rathus", "higher", "Pre", "Follow-up", xG1)$worsened,
#   clinsigraw("rathus", "higher", "Pre", "Follow-up", xG2)$worsened
# ), method="bonferroni", altp=TRUE)
# No worsening in any group, nothing to test!

phq_sum.pre.fu.kw <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
phq_sum.pre.fu.kw.harmed <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG3)$harmed,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG1)$harmed,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 1.2129, df = 2, p-value = 0.55

phq_sum.pre.fu.kw.worsened <- dunn.test(list(
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG3)$worsened,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG1)$worsened,
  clinsigraw("phq_sum", "lower", "Pre", "Follow-up", xG2)$worsened
), method="bonferroni", altp=TRUE)
...
gad_sum.pre.fu.kw <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 5.123, df = 2, p-value = 0.08

gad_sum.pre.fu.kw.harmed <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG3)$harmed,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG1)$harmed,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.1993, df = 2, p-value = 0.33

gad_sum.pre.fu.kw.worsened <- dunn.test(list(
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG3)$worsened,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG1)$worsened,
  clinsigraw("gad_sum", "lower", "Pre", "Follow-up", xG2)$worsened
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.5818, df = 2, p-value = 0.28

lsas_sum.pre.fu.kw <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG3)$recovered,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG1)$recovered,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG2)$recovered
), method="bonferroni", altp=TRUE)
...
lsas_sum.pre.fu.kw.harmed <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG3)$harmed,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG1)$harmed,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG2)$harmed
), method="bonferroni", altp=TRUE)
# Kruskal-Wallis chi-squared = 2.0156, df = 2, p-value = 0.37

lsas_sum.pre.fu.kw.worsened <- dunn.test(list(
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG3)$worsened,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG1)$worsened,
  clinsigraw("lsas_sum", "lower", "Pre", "Follow-up", xG2)$worsened
), method="bonferroni", altp=TRUE)
...
Both treatment groups showed sustained within-group effects on assertiveness at 1 year Follow-up when compared to the Pre-treatment measurement time point, measured with AAA-S Adaptive and the RAS, suggesting that assertive behaviors were still manifest well beyond the end of participation in the intervention. However, only in the Guided self-help group were significant Post to Follow-up comparisons found, for measures AAA-S Adaptive and the RAS, `r report.text2(aaas_ad.out, "Post Support - (Follow-up Support)", -1)` and `r report.text2(rathus.out, "Post Support - (Follow-up Support)", -1)` respectively. In the Self-help group, there was no difference between Post and Follow-up time points for either measure, from `r xprnot(rathus.out$emmpairs$p.value[41])` to `r xprnot(aaas_ag.out$emmpairs$p.value[55])`. This indicates, with regards to adaptive assertiveness, that therapist support provided some benefit to participants' ability to generalize behaviors beyond the duration of the intervention. Table 1 summarizes significance tests of estimated marginal mean differences and effect sizes, including 95\% confidence intervals.
...
RAS, `r report.kw(rathus.pre.post.kw)`, and LSAS-SR, `r report.kw(lsas_sum.pre.post.kw)`. However, there was no difference between groups at Post with regards to AAA-S Aggressive, `r report.kwnot(aaas_ag.pre.post.kw)`, PHQ-9, `r report.kwnot(phq_sum.pre.post.kw)`, or GAD-7, `r report.kwnot(gad_sum.pre.post.kw)`. At Follow-up, the difference in recovery rates between conditions was significant for both behavioral and syndromal measures: AAA-S Adaptive, `r report.kw(aaas_ad.pre.fu.kw)`; RAS, `r report.kw(rathus.pre.fu.kw)`; PHQ-9, `r report.kw(phq_sum.pre.fu.kw)`; and LSAS-SR, `r report.kw(lsas_sum.pre.fu.kw)`; but not for AAA-S Aggressive, `r report.kwnot(aaas_ag.pre.fu.kw)` or GAD-7, `r report.kwnot(gad_sum.pre.fu.kw)`. See Table 2 for a summary of clinical recovery rates and the significance of pairwise tests against Waitlist. 

Notably, a number of participants, `r clinsigtext("aaas_ad", "higher", "Pre", "Post", "Self-help", aaas_ad.pre.post.kw)[1]` in the Self-help group enjoyed clinical recovery with regards to adaptive assertiveness measured with AAA-S Adaptive at Post, increasing to `r clinsigtext("aaas_ad", "higher", "Pre", "Follow-up", "Self-help", aaas_ad.pre.fu.kw)[1]` at Follow-up. In the Guided self-help group the number of recovered participants at Post was non-significantly different from that of the Waitlist, while significantly different at Followup, `r clinsigtext("aaas_ad", "higher", "Pre", "Follow-up", "Support", aaas_ad.pre.fu.kw)[1]`. For assertiveness assessed with the RAS, the corresponding numbers and rates were `r clinsigtext("rathus", "higher", "Pre", "Post", "Self-help", rathus.pre.post.kw)[1]` for Self-help and `r clinsigtext("rathus", "higher", "Pre", "Post", "Support", rathus.pre.post.kw)[1]` for Guided self-help at Post, increasing to `r clinsigtext("rathus", "higher", "Pre", "Follow-up", "Self-help", rathus.pre.fu.kw)[1]` and `r clinsigtext("rathus", "higher", "Pre", "Follow-up", "Support", rathus.pre.fu.kw)[1]` respectively at Follow-up. Thus, the clinical significance findings are in agreement with the statistical analysis of change, confirming that adaptive expressions of assertiveness increased Pre to Post in both treatment groups. Clinical recovery measured with PHQ-9 Pre to Post indicate that the intervention, compared to Waitlist, was effective in moving a number of participants out of depression in the Guided self-help group, `r clinsigtext("phq_sum", "lower", "Pre", "Post", "Support", phq_sum.pre.post.kw)[1]`, but not so in Self-help group, pointing to that interaction with therapists might contribute to faster recovery for depressed participants. In addition, a significant number of participants in the Self-help group recovered from symptoms of social anxiety as captured with LSAS-SR, `r clinsigtext("lsas_sum", "lower", "Pre", "Post", "Self-help", lsas_sum.pre.post.kw)[1]` at Post, increasing to `r clinsigtext("lsas_sum", "lower", "Pre", "Follow-up", "Self-help", lsas_sum.pre.fu.kw)[1]` at Follow-up. In the Guided self-help condition, there was no significant number of recovered participants compared to Waitlist at Post, but `r clinsigtext("lsas_sum", "lower", "Pre", "Follow-up", "Support", lsas_sum.pre.fu.kw)[1]` at Follow-up. These clinical findings seem to pick up that symptoms of social anxiety continued to subside in the year following treatment, to the point that some participants had recovered. Combined with the clinically relevant recovery for assertive behaviors measured with AAA-S Adaptive and RAS, this could be a sign of generalization of assertive behaviors having continued to occur after the end of treatment, facilitating extinction of anxiety responses.

#### Reliable harm and deterioration

Checking for any signs of reliable harm, the most severe form of deterioration, a significant difference between groups in clinical significant change was found for the AAA-S Aggressive measure, `r report.kw(aaas_ag.pre.post.kw.harmed)`, as $n$ = $3$ participants ($4$\%) had reached below the threshold for the category 'Harmed' while waiting to enter into treatment (see Table 2). However, post hoc testing did not reveal any significant pairwise differences between any of the treatment groups and the Waitlist group, `r xprnot(aaas_ag.pre.post.kw.harmed$altP.adjusted[2])` and `r xprnot(aaas_ag.pre.post.kw.harmed$altP.adjusted[1])`. No other instances of suspected reliable harm Pre to Post or Pre to Follow-up were identified for either measure. The next worst category in assessing reliable change is 'Deteriorated'; in order to identify any possible cases of reliable deterioration _and_ reliable harm, these two categories were collapsed into the conservative ad hoc category 'Worsened' -- gathering participants that had moved into either of these two categories. Using the 'Worsened' portmanteau category, and checking clinical change Pre to Post treatment, we observed a significant difference between groups for LSAS-SR, `r report.kw(lsas_sum.pre.post.kw.worsened)`, meriting a follow-up pairwise comparison. It revealed that the number of participants in the Waitlist condition that had either reached 'Deteriorated' or 'Harmed', going from Pre to Post treatment, was significant compared to the Guided self-help group, $n$ = 7 ($10$\xspace\%), $Z$($2$) = `r xresult(lsas_sum.pre.post.kw.worsened$Z[2],2)`, `r xpr(lsas_sum.pre.post.kw.worsened$altP.adjusted[2])`. The same held true comparing Pre to Follow-up, `r report.kw(lsas_sum.pre.fu.kw.worsened)`, where these $7$ individuals were significantly more than the number of worsened participants in both the Self-help group, $Z$($2$) = `r xresult(lsas_sum.pre.fu.kw.worsened$Z[1],2)`, `r xpr(lsas_sum.pre.fu.kw.worsened$altP.adjusted[1])`, and the Guided self-help group, $Z$($2$) = `r xresult(lsas_sum.pre.fu.kw.worsened$Z[2],2)`, `r xpr(lsas_sum.pre.fu.kw.worsened$altP.adjusted[2])`. Finally, a difference between groups was found for PHQ-9, `r report.kw(phq_sum.pre.fu.kw.worsened)`, with post hoc testing revealing a difference for the Guided self-help group, $n$ = 12 ($18$\xspace\%), $Z$($2$) = `r xresult(phq_sum.pre.fu.kw.worsened$Z[1],2)`, `r xpr(phq_sum.pre.fu.kw.worsened$altP.adjusted[1])`. These findings reveal that the non-active Waitlist condition brought about adverse clinical change for $10$\% of participants with regards to social anxiety and $18$\% with regards to depression, highlighting the need to use caution comparing results for the treatment groups with those of the Waitlist condition, lest running the risk of artificially inflated effect sizes (especially for PHQ-9 for which estimated marginal means in fact increased over time; see Figure 3).

### Summary of findings

To summarize, we found large between-group effects on assertiveness of Self-help and Guided self-help participation in Respekt^2^, compared to Waitlist control. For AAA-S Adaptive and RAS at Post in the Self-help condition, `r report.text2(aaas_ad.out, "Post Waitlist - (Post Self-help)", -1,"effshort")` and `r report.text2(rathus.out, "Post Waitlist - (Post Self-help)", -1,"effshort")` respectively. The corresponding effect sizes in the Guided self-help group was `r report.text2(aaas_ad.out, "Post Waitlist - Post Support", -1,"effshort")` and `r report.text2(rathus.out, "Post Waitlist - Post Support", -1,"effshort")` respectively. Within-group effects from Pre to Post were also large: `r report.text2(aaas_ad.out, "(Pre Self-help) - (Post Self-help)", -1,"effshort")` for AAA-S Adaptive in the Self-group, and `r report.text2(aaas_ad.out, "Pre Support - Post Support", -1,"effshort")` in the Guided self-help group; `r report.text2(rathus.out, "(Pre Self-help) - (Post Self-help)", -1,"effshort")` for the RAS in the Self-group, and `r report.text2(rathus.out, "Pre Support - Post Support", -1,"effshort")` in the Guided self-help group. For AAA-S Adaptive in the Guided self-group, the `r report.text2(rathus.out, "(Post Self-help) - (Follow-up Self-help)", -1,"effshort")` increase at 1 year Follow-up was also significant. 
<!-- No significant time \texttimes{} group interaction was found for AAA-S Aggressive.  -->
Clinically speaking, $n$ = 15 (22%) of participants in the Self-help group had recovered with regards to AAA-S Adaptive at Follow-up, while $n$ = 18 (26%) of participants in the Guided self-help group had recovered at the same time point. The corresponding recovery rates for RAS were $n$ = 21 (31%) and $n$ = 25 (36%). We conclude that Respekt^2^ did indeed help participants act more assertively, although not more aggressively, and that the effects were either sustained or more pronounced 1 year after the intervention.

Assessing the effect of the Respekt^2^ intervention on symptom burden with respect to depression and common anxiety disorders, we found no significant effects at Post for PHQ-9 or LSAS-SR in the Self-help group, compared with Waitlist. At Follow-up, however, we found a large between-group effect on LSAS-SR, `r report.text2(lsas_sum.out, "During Waitlist - (Follow-up Self-help)", 1,"effshort")`. Effects were also found in the Guided self-help group, where participants were supported by therapists: Compared with waitlist, participants exhibited moderate effects at Post for PHQ-9, `r report.text2(phq_sum.out, "Post Waitlist - Post Support", 1,"effshort")`, and LSAS-SR, `r report.text2(lsas_sum.out, "Post Waitlist - Post Support", 1,"effshort")` respectively, as well as a large effect only for LSAS-SR at Follow-up, `r report.text2(lsas_sum.out, "During Waitlist - (Follow-up Support)", 1,"effshort")`. 
<!-- There was no significant time \texttimes{} group interaction for generalized anxiety, as measured with GAD-7, however.  -->
Within-group comparisons revealed moderate effects in the Self-help group for LSAS-SR, `r report.text2(lsas_sum.out, "(Pre Self-help) - (Post Self-help)", 1,"effshort")` Pre--Post, and large effects `r report.text2(lsas_sum.out, "(Pre Self-help) - (Follow-up Self-help)", 1,"effshort")` Pre--Follow-up. Similar effects were found in the Guided self-help group, `r report.text2(lsas_sum.out, "Pre Support - Post Support", 1,"effshort")` Pre--Post, and `r report.text2(lsas_sum.out, "Pre Support - (Follow-up Support)", 1,"effshort")` Pre--Follow-up. A moderate effect was found for PHQ-9 Pre--Follow-up for the Guided self-help group, `r report.text2(phq_sum.out, "Pre Support - (Follow-up Support)", 1,"effshort")`. These findings were supplemented by the analysis of clinical significant change, where, $n$ = 14 (21%) had recovered from social anxiety at Follow-up in the Self-help group, and $n$ = 18 (26%) in the group that were guided by therapists. In addition, $n$ = 18 (26%) in the Guided self-help group had recovered from depression at Follow-up. Thus, we found that Respekt^2^ clearly reduced symptoms of social anxiety across both treatment conditions, and over time to a moderate degree reduced symptoms of depression among participants that were guided by therapists.

# Discussion

The current study advances current knowledge about a stand-alone transdiagnostic intervention targeting assertiveness, Respekt^2^, providing evidence for its effects on levels of assertive behaviors, and consequently its effects on symptoms of common psychiatric disorders of anxiety and depression. The intervention was found to be effective at increasing levels of assertiveness, achieving large effects in both the Self-help and Guided self-help groups, $ES$ = ...

<!-- . Assertiveness has been long neglected in clinical research, despite the fact that lack of assertiveness have been linked to many psychiatric syndromes, including generalized anxiety, emotional instability, social anxiety, and depression, as well as problems in relationships and with work-related stress [@Speed2018a]. We found support, and large effects,  -->

<!-- Assertiveness training fell the wayside both as stand-alone intervention in clinical practice and in clinical research in the late 1990'ies, when research had come to focus on specific psychiatric syndromes and their corresponding treatments rather than transdiagnostic interventions [@Speed2018a]. At the same time, unassertive behaviors play an important role in a multitude of disorders, and  -->

<!-- , and its effects on levels of assertiveness and its effects on measures commonly used within psychiatric settings to assess symptoms of depression, generalized anxiety and social anxiety, as it was applied to a non-clinical population. Specifically, ... -->
...
Restate effects between groups, compare to literature for iCBT, @Carlbring2018 (iCBT as effective as face-to-face treatment), @Andrews2018 (iCBT effective, acceptable, average between-group effect size g = 0.8 compared to controls; results from @Baumeister2014a concur, also pointing to guided iCBT producing somewhat larger effects than non-guided). Also, compare w/ frequencies or reliable change, e.g. @Rozental2015.

Relate to @Baumeister2014a: Being guided by a therapist in Respekt^2^ does seem to entail some advantages, especially for longer-term outcomes. In the Guided self-help group, adaptive assertiveness had increased between end of treatment and 1-year Follow-up, and depression had decreased from pre-treatment to 1-year Follow-up. These phenomena were not observed in the group where the participants were engaging with the intervention on their own. 

(Speculatively, this might be explained by either increased _willingness_ to participate in behavioral experiments through the effects of coaching, an increased _ability to engage_ in behavioral experiments through active problem solving with the therapist, or increased _ability to generalize_ findings from such experiments through better compliance to homework assignments, or any combination thereof. To dismantle the difference ... (Följa upp på något sätt? Kolla antal inloggningar? Antal meddelanden som skickats? Något för en annan artikel? Hur som helst, kolla litteraturen, jämför med andras fynd.)

@Johansson2012 "In this review, the authors have found an evidence for a strong correlation between the degree of support and outcome"

@Mohr2011 "We argue that human support increases adherence through accountability to a coach who is seen as trustworthy, benevolent, and having expertise")

Limitations: Although the effect sizes between groups and within treatment groups were substantial in the Respekt^2^ intervention, it is difficult to transfer the findings to the general population. Firstly, X, secondly, Y, ... The current study also suffers from a number of limitations, among them Å (operationalization of 'assertiveness'), Ä (no validated scales) and Ö (waitlist as control).

Future research: However, it is intriguing that ..., and merits further investigation. Directions for future research could include ...
