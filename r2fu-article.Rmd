...
# aaas_ad.effsizes <-
#   c(
#     within.effsize(aaas_ad.lmer,29,-1,8),   #Pre Self-help - Post Self-help
#     within.effsize(aaas_ad.lmer,30,-1,9),   #Pre Self-help - Follow-up Self-help
#     within.effsize(aaas_ad.lmer,51,-1,14),  #Pre Support - Post Support
#     within.effsize(aaas_ad.lmer,52,-1,15),  #Pre Support - Follow-up Support
#     within.effsize(aaas_ad.lmer,2,-1,2),    #Pre Waitlist - Post Waitlist
#     between.effsize(aaas_ad.lmer,46,-1,7),  #Waitlist Post - Self-help Post
#     between.effsize(aaas_ad.lmer,48,-1,NULL), #Waitlist Post - Self-help Follow-up
#     "(idem)",
#     between.effsize(aaas_ad.lmer,47,-1,8),  #Waitlist Post - Support Post
#     between.effsize(aaas_ad.lmer,49,-1,NULL), #Waitlist Post - Support Follow-up
#     "(idem)",
#     between.effsize(aaas_ad.lmer,50,-1,9),  #Self-help Post - Support Post
#     between.effsize(aaas_ad.lmer,55,-1,12)  #Self-help Follow-up - Support Follow-up
#   )
# 
# aaas_ag.effsizes <-
#   c(
...
aaas_ag.effsizes.OFF <-
  c(
    within.effsize(aaas_ag.lmer,29,-1,40),  # (Pre Self-help) - (Post Self-help)
    within.effsize(aaas_ag.lmer,30,-1,41),  # (Pre Self-help) - (Follow-up Self-help)
    within.effsize(aaas_ag.lmer,51,-1,62),  # Pre Support - Post Support
    within.effsize(aaas_ag.lmer,52,-1,63),  # Pre Support - (Follow-up Support) 
    within.effsize(aaas_ag.lmer,2,-1,2),    # Pre Waitlist - Post Waitlist
    between.effsize(aaas_ag.lmer,46,-1,25), # Post Waitlist - (Post Self-help)
    between.effsize(aaas_ag.lmer,48,-1,26), # Post Waitlist - (Follow-up Self-help)
    "(idem)",
    between.effsize(aaas_ag.lmer,47,-1,29), # Post Waitlist - Post Support 
    between.effsize(aaas_ag.lmer,49,-1,30), # Post Waitlist - (Follow-up Support)
    "(idem)",
    between.effsize(aaas_ag.lmer,50,-1,55), # (Post Self-help) - Post Support
    between.effsize(aaas_ag.lmer,55,-1,60)  # (Follow-up Self-help) - (Follow-up Support)
  )
...
gad_sum.effsizes.OFF <-
  c(
    within.effsize(gad_sum.lmer,29,1,40),  # (Pre Self-help) - (Post Self-help)
    within.effsize(gad_sum.lmer,30,1,41),  # (Pre Self-help) - (Follow-up Self-help)
    within.effsize(gad_sum.lmer,51,1,62),  # Pre Support - Post Support
    within.effsize(gad_sum.lmer,52,1,63),  # Pre Support - (Follow-up Support) 
    within.effsize(gad_sum.lmer,2,1,2),    # Pre Waitlist - Post Waitlist
    between.effsize(gad_sum.lmer,46,1,25), # Post Waitlist - (Post Self-help)
    between.effsize(gad_sum.lmer,48,1,26), # Post Waitlist - (Follow-up Self-help)
    between.effsize(gad_sum.lmer,48,1,17), # During Waitlist - (Follow-up Self-help) ; levels(r2fu.long.data$time)[which.min(gad_sum.emm.plot$data$predicted[c(1,4,7)])]
    between.effsize(gad_sum.lmer,47,1,29), # Post Waitlist - Post Support 
    between.effsize(gad_sum.lmer,49,1,30), # Post Waitlist - (Follow-up Support)
    between.effsize(gad_sum.lmer,48,1,21), # During Waitlist - (Follow-up Support); levels(r2fu.long.data$time)[which.min(gad_sum.emm.plot$data$predicted[c(1,4,7)])]
    between.effsize(gad_sum.lmer,50,1,55), # (Post Self-help) - Post Support
    between.effsize(gad_sum.lmer,55,1,60)  # (Follow-up Self-help) - (Follow-up Support)
  )
...
<!-- Lägga till During i tabell? -->
<!-- Lägga till Post to Fu i tabell? -->
<!-- Förenkla/bygga om within.group och between.group -->
<!-- Slår vi i taket för någon skala, ex RAS? Jämför med normer -->

### Within-group effects

For the primary transdiagnostic measures of skillful behaviors it is notable that both treatment groups Self-help and Guided self-help improved significantly during the course of the intervention, showing large within-group effects with regards to AAA-S Adaptive from Pre to Post, `r report.text(aaas_ad.lmer, 29, -1, 40)`, and `r report.text(aaas_ad.lmer, 51, -1, 62)` respectively. No Pre to Post effect was found for the Waitlist condition, `r xprnot(rout(aaas_ad.lmer)$emmpairs$p.value[2])`. For the RAS, significant large effects on assertiveness were also found for the Self-help group, `r report.text(rathus.lmer, 29, -1, 40)`, and the Guided self-help group, `r report.text(rathus.lmer, 51, -1, 62)`. As with AAA-S Adaptive, means in the Waitlist condition did not change between Pre and Post with regards to the RAS, nor with any other remaining measure, all `r xprnot(rout(rathus.lmer)$emmpairs$p.value[2])`. Both treatment groups showed sustained within-group effects for on assertiveness at 1 year Follow-up when compared to the Pre-treatment measurement time point, measured with AAA-S Adaptive and the RAS, suggesting that assertive behaviors still were manifest well after the end of participation in the treatment groups. However, only for X was the Post to Follow-up comparison in itself significant, indicating that there is generally no change between Post and Follow-up ...(utveckla). (See Table 1 for a summary of effect sizes and significance tests of estimated marginal mean differences). 

Among the secondary measures of syndromal symptoms ... (lyft viss effekt för depression mätt med PHQ-9 at Follow-up, lyft stor effekt på symtom för social ångest) (see Table 1 for a summary of effect sizes). 

Adding to the picture that assertive behaviors increased and symptoms of depression and social phobia decreased as a consequence of participation in the intervention, there was a significant difference between the groups at Post in the number of participants that had recovered, that is had moved across the cutoff for reliable and clinically significant change: AAA-S Adaptive, `r report.kw(aaas_ad.pre.post.kw)`;
RAS, `r report.kw(rathus.pre.post.kw)`; PHQ-9, `r report.kw(phq_sum.pre.post.kw)`; and LSAS-SR,  `r report.kw(lsas_sum.pre.post.kw)`. Recovery rates between conditions were also significant at Follow-up for AAA-S Adaptive, `r report.kw(aaas_ad.pre.fu.kw)`;
RAS, `r report.kw(rathus.pre.fu.kw)`; PHQ-9, `r report.kw(phq_sum.pre.fu.kw)`; and LSAS-SR,  `r report.kw(lsas_sum.pre.fu.kw)`. However, there was no difference between groups for AAA-S Aggressive, `r report.kw(aaas_ag.pre.post.kw)`, and GAD-7, `r report.kw(gad_sum.pre.post.kw)` at Post, nor at Follow-up, `r report.kw(aaas_ag.pre.fu.kw)` and `r report.kw(gad_sum.pre.fu.kw)`. See Table 2 for a summary of cutoff values, clinical recovery rates and the significance of pairwise post hoc tests. The clinical significance findings are in agreement with the statistical analysis of change, confirming that adaptive expressions of assertiveness captured with AAA-S Adaptive and the RAS increased Pre to Post, and that the trends continued up to the 1-year Follow-up time point (while aggressive expressions had not changed at neither Post or Follow-up). Also, depressive symptoms captured with PHQ-9 had decreased significantly only for the Guided self-help group, and only at Follow-up. Symptoms of generalized anxiety, however, decreased significantly for all four combinations of Post and Follow-up time points, and Self-help and Guided self-help groups.

`r clinsig("aaas_ad", "higher", "Pre", "Post", xG3, aaas_ad.pre.post.kw)[1]`

<!-- Move chi2 tests to table? -->
<!-- Add cutoff values to table 2 -->


<!-- AAA-S Adaptive from Pre to Post and from Pre to Follow-up, `r report.kw(aaas_ad.pre.post.kw)` and `r report.kw(aaas_ad.pre.fu.kw)` respectively. , RAS, and LSAS-SR;  -->

<!-- in the Self-help group X (n, %), Y, and Z at Post treatment, and X2, Y2, and Z2 at 1 year Follow-up; in the Guided self-help group X3, Y3, and Z3 at Post treatment, and X4, Y4, and Z4 at 1 year Follow-up. For the PHQ-9 measure,  -->

### Between-group effects

Compared to the Waitlist condition, both the Self-help and the Guided self-help conditions showed significant and large effects at the Post timepoint, ... (utveckla)
