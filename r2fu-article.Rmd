...
Mixed models for two of three primary measures revealed time \texttimes{} group interaction effects: The Adaptive subscale of AAA-S, `r report.anova(aaas_ad.anova)[3]`, and the RAS, `r report.anova(rathus.anova)[3]`. The model for Aggressive subscale of AAA-S  did not, `r report.anova(aaas_ag.anova)[3]`. This suggests that self-assessments of assertive behaviors increased during the course of the intervention, while behaviors that partipants deemed "over the top" in communication style or intensity (either putting the relationship and/or the person’s self-respect at risk, or simply overstepping the boundaries of the person’s comfort zone), did not.
...
Similarily large effect sizes were found for RAS, $\Delta M$ between Pre and Post time points was `r xresult(rout(rathus.lmer)$emmpairs.by.group$estimate[8] * -1,1)`, $t$\(_{`r xresulttext(rout(rathus.lmer)$emmpairs.by.group$df[8],0)`}\) = `r xresult(rout(rathus.lmer)$emmpairs.by.group$t.ratio[8],1)`, `r xpr(rout(rathus.lmer)$emmpairs.by.group$p.value[8])`, $ES$ = `r xresult(rout(rathus.lmer)$eff.by.group$effect.size[29] * -1,1)`, 95% CI [`r xresult(rout(rathus.lmer)$eff.by.group$upper.CL[29] * -1,1)`, `r xresult(rout(rathus.lmer)$eff.by.group$lower.CL[29] * -1,1)`]. The estimated $\Delta M$ between Pre and Follow-up time points was also pronounced, `r xresult(rout(rathus.lmer)$emmpairs.by.group$estimate[9] * -1,1)`, $t$\(_{`r xresulttext(rout(rathus.lmer)$emmpairs.by.group$df[9],0)`}\) = `r xresult(rout(rathus.lmer)$emmpairs.by.group$t.ratio[9],1)`, `r xpr(rout(rathus.lmer)$emmpairs.by.group$p.value[9])`, $ES$ = `r xresult(rout(rathus.lmer)$eff.by.group$effect.size[30] * -1,1)`, 95% CI [`r xresult(rout(rathus.lmer)$eff.by.group$upper.CL[30] * -1,1)`, `r xresult(rout(rathus.lmer)$eff.by.group$lower.CL[30] * -1,1)`].
