...
The moderate to large between-group effects on assertiveness found in Respekt^2^ at Post in the Unguided self-help condition, from `r report.text2(aaas_ag.out, paste(xT3,xG3,"-",xT3,xG1), -1,"effshort")` to `r report.text2(rathus.out, paste(xT3,xG3,"-",xT3,xG1), -1,"effshort")` (with slightly but insignificantly larger effects in the Guided group), are comparable to those found in clinical trails of iCBT interventions for  other trans-diagnostic behavioral targets such as procrastination ($d$ = ??) [@Rozental2015], and perfectionism ($d$ = ??) [@Rozental2017], showing that the 'assertiveness' construct can be used successfully as a target in behavior therapy, helping participants appreciate and self-report meaningful change in concrete actions.

[Also, compare w/ frequencies or reliable change, e.g. @Rozental2015.]

#### Effects on psychiatric symptoms

However, at Follow-up, we found a large between-group effect on LSAS-SR, `r report.text2(lsas_sum.out, paste(xT2,xG3,"-",xT4,xG1), 1,"eff")`. Effects were also found in the Guided self-help group, where participants were supported by therapists: Comparing with the Waitlist group, participants exhibited moderate effects at Post for PHQ-9, `r report.text2(phq_sum.out, paste(xT3,xG3,"-",xT3,xG2), 1,"eff")`, and LSAS-SR, `r report.text2(lsas_sum.out, paste(xT3,xG3,"-",xT3,xG2), 1,"eff")` respectively, as well as a large effect only for LSAS-SR at Follow-up, `r report.text2(lsas_sum.out, paste(xT2,xG3,"-",xT4,xG2), 1,"eff")`. These are in agreement with those for iCBT in general, where the average between-group effect size is $g$ = $.80$ compared to controls [@Andrews2018], which in turn is about the same as for face-to-face treatment [@Carlbring2018].
